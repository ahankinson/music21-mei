...
class ConverterMei(object):
    ...
    def _parse_score(self):
        ...
        for st in stfdef:
            ...
            if stid not in self._staff_registry.keys():
                # self._staff_registry[stid] = st
                self._register_clef(st)
        ...
        for staff in self._structure.keys():
            stg = spanner.StaffGroup()
            self._score.append(stg)
            
            for voice in self._structure[staff].keys():
                pt = stream.Part()
                stg.addComponents(pt)
                self._score.append(pt)
                
                for measure in sorted(self._structure[staff][voice].keys()):
                    ...
                    for el in self._structure[staff][voice][measure]:
                        ...
                        if el.name == "note":
                            ...
                            for note in el.children:
                                self._measure_registry[mid].append(self._create_note(note))
